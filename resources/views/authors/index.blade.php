        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('authors.index') }}" class="mt-6">
                        @csrf
                    ...
                    @forelse ($authors as $author)
                        <a href="{{ route('authors.show', $author) }}" class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100">
                            ...
                            <div class="flex flex-col justify-between p-4 leading-normal">
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">{{ $author->name }}</h5>
                                <p class="mb-3 font-normal text-gray-700">{{ Str::limit($author->bio, 20) }}</p>
                            </div>
                        </a>
                    @empty
                        No authors yet
